...
class TestFiredpyArgumentParser(unittest.TestCase):
    ...
    def test_load_params(self):
        ...
        with patch("builtins.open", mock_open(read_data=data)):
            parser = FiredpyArgumentParser()
            self.assertIn("arg1", parser.args)
            self.assertEqual(parser.args["arg1"]["prompt"], "Test Prompt")
            self.assertEqual(parser.args["arg1"]["type"], "int")
            self.assertEqual(parser.args["arg1"]["last_value"], "5")
            self.assertIsNone(parser.args["arg1"]["accepted_values"])
    ...
    def test_prompt_for_str_unacceptable_value(self, mock_input):
        ...
        with patch("builtins.open", mock_open(read_data=data)):
            parser = FiredpyArgumentParser()
            self.assertEqual(parser.args["arg4"]["last_value"], "default")
            response = parser.prompt_for_argument("arg4")
            self.assertEqual(2, mock_input.call_count)
            self.assertEqual(response, "acceptable")
            self.assertEqual(parser.args["arg4"]["last_value"], "acceptable")
    ...
    def test_prompt_for_str_sensitive(self, mock_input):
        ...
        with patch("builtins.open", mock_open(read_data=data)):
            parser = FiredpyArgumentParser()
            self.assertEqual(parser.args["arg5"]["last_value"], "")
