...
def write_output_report(output_dir: str, tilename, file_base, input_name, start_date: str, end_date: str, daily, spatial_param, temporal_param, shapefile, shp_type):
    read_path = os.path.join(output_dir, 'outputs', f"{file_base}_output_report.txt")
    first_date_str = format_date(first_date).strftime("%B %Y")
    last_date_str = format_date(last_date).strftime("%B %Y")
    name = format_name(input_name, tilename)

    with open(read_path, "w") as text_file:
        write_abstract(text_file, name, first_date_str, last_date_str, spatial_param, temporal_param)
        write_general_info(text_file, name, first_date_str, last_date_str)
        write_data_file_overview(text_file, daily, shapefile, shp_type, file_base, last_date_str)
        write_methodological_info(text_file, spatial_param, temporal_param)
        write_data_specific_info(text_file, proj_dir, file_base, shp_type)
